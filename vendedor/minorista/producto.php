...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action'] ?? '') === 'toggle_source') {
  ...
  if (!$ppId) $err = "Elegí un mayorista.";
  else {
    $pdo->prepare("INSERT INTO store_product_wholesale_sources(store_product_id,wholesale_store_product_id,enabled)
...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action'] ?? '') === 'unlink_source') {
  ...
  if (!$ppId) $err = "Producto inválido.";
    ...
    $pdo->prepare("DELETE FROM store_product_wholesale_sources WHERE store_product_id=? AND wholesale_store_product_id=? LIMIT 1")
        ->execute([$productId,$ppId]);
...
$linkedSt = $pdo->prepare("
  ...
  JOIN store_products sp ON sp.id = sps.wholesale_store_product_id
  JOIN stores s ON s.id = sp.store_id
  JOIN sellers sel ON sel.id = s.seller_id
  LEFT JOIN store_product_sources spsp ON spsp.store_product_id = sp.id AND spsp.enabled=1
  LEFT JOIN warehouse_stock ws ON ws.provider_product_id = spsp.provider_product_id
...
$linkedProductsRaw = $linkedSt->fetchAll();
$linkedProducts = [];
$linkedStockTotal = 0;
foreach ($linkedProductsRaw as $linked) {
  $stock = (int)$linked['own_stock_qty'] + (int)$linked['provider_stock'];
  $store = [
    'markup_percent' => $linked['markup_percent'],
    'store_type' => 'wholesale',
  ];
  $price = current_sell_price($pdo, $store, $linked);
  $linkedProducts[] = [
    'id' => (int)$linked['id'],
    'title' => (string)$linked['title'],
    'sku' => (string)($linked['sku'] ?? ''),
    'universal_code' => (string)($linked['universal_code'] ?? ''),
    'seller_name' => (string)($linked['seller_name'] ?? ''),
    'store_name' => (string)($linked['store_name'] ?? ''),
    'stock' => $stock,
    'price' => $price,
  ];
  $linkedStockTotal += $stock;
}
$provStock = $linkedStockTotal;
...
echo "
<p>Stock mayorista: ".h((string)$provStock)." | Precio actual (".h($priceSourceLabel)."): ".h($sellTxt)." (total: ".h((string)$stockTotal).")</p>
<form method='post'>
<input type='hidden' name='csrf' value='".h(csrf_token())."'>
...
if (!$linkedProducts) {
  ...
  foreach($linkedProducts as $linked){
    $providerName = $linked['seller_name'] ?: '—';
    $universalCode = $linked['universal_code'] ?: '—';
    $price = $linked['price'] > 0 ? '$'.number_format((float)$linked['price'], 2, ',', '.') : '—';
    echo "<tr>
...
(function() {
  ...
  function renderResults(items, emptyReason) {
    ...
    const rows = items.map(function(item) {
      return "<tr data-id='" + item.id + "'>" +
        "<td>" + escapeHtml(item.wholesale_name || '—') + "</td>" +
  ...
  function fetchResults(query) {
    ...
    fetch('/vendedor/minorista/api/wholesale_products_search.php?' + params.toString(), {
  ...
  function addLinkedRow(item) {
    ...
    const providerName = item.wholesale_name || '—';
  ...
  function linkProviderProduct(linkedId, rowEl) {
    ...
    fetch('/vendedor/minorista/api/link_wholesale_product.php', {
      method: 'POST',
      headers: {
