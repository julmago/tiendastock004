...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  $pairs = [
    'retail_fee_percent' => (string)($_POST['retail_fee_percent'] ?? '3.00'),
    'wholesale_fee_percent' => (string)($_POST['wholesale_fee_percent'] ?? '2.00'),
...
$cur = [];
foreach(['retail_fee_percent','wholesale_fee_percent','provider_fee_percent','mp_extra_percent'] as $k){
  $cur[$k] = setting($pdo, $k, '');
}
...
<p>Comisión minorista (%): <input name='retail_fee_percent' value='".h($cur['retail_fee_percent'])."'></p>
<p>Comisión mayorista (%): <input name='wholesale_fee_percent' value='".h($cur['wholesale_fee_percent'])."'></p>
